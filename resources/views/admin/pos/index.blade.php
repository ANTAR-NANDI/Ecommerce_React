...
@if($draft)<div id="pos-draft-data" data-payment="{{ $draft->payment_method_id ?: ($draft->payment_method === 'Due' ? 'due' : '') }}" data-items='{{ $draft->items->map(function($item) { return ['product_id'=>$item->product_id,'product_name'=>$item->product_name,'quantity'=>(float)$item->quantity,'unit_price'=>(float)$item->unit_price]; })->toJson() }}'></div>@endif
@endsection
