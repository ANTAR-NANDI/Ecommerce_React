...
@section('content')
@php($selectedWarehouseId = old('warehouse_id', $draft?->warehouse_id))
@if($errors->any())
...
<div class="row g-4">
    ...
    <div class="col-xl-4"><div class="panel h-100">
        ...
        <div class="border-top mt-3 pt-3"><div class="row g-2 mb-3"><div class="col-12"><label class="form-label">Customer</label><select id="pos-customer-id" class="form-select"><option value="">Walk-in Customer</option>@foreach($customers as $customer)<option value="{{ $customer->id }}" data-name="{{ $customer->full_name }}" data-phone="{{ $customer->phone }}" @selected(old('customer_id', $draft?->customer_id)==$customer->id)>{{ $customer->full_name }}{{ $customer->phone ? ' — '.$customer->phone : '' }}</option>@endforeach</select><div class="form-text">Choose Walk-in Customer for an anonymous cash sale, or select a registered customer for a due sale.</div></div><div id="pos-walk-in-fields" class="col-12"><div class="row g-2"><div class="col-12"><input id="pos-customer-name" class="form-control" value="{{ old('customer_name', $draft?->customer_name) }}" placeholder="Walk-in customer name (optional)"></div><div class="col-12"><input id="pos-customer-phone" class="form-control" value="{{ old('customer_phone', $draft?->customer_phone) }}" placeholder="Walk-in customer phone (optional)"></div></div></div><div class="col-12"><label class="form-label">Payment method</label><select id="pos-payment-method" class="form-select">@foreach($paymentMethods as $method)<option value="{{ $method->id }}" @selected($draft?->payment_method_id===$method->id)>{{ $method->name }}</option>@endforeach<option value="due" @selected($draft?->payment_method==='Due')>Due — Customer Receivable</option></select></div></div>
<div class="row g-2 mb-3">
    <div class="col-6"><label for="pos-discount-type" class="form-label">Discount type</label><select id="pos-discount-type" class="form-select"><option value="fixed" @selected(old('discount_type', $draft?->discount_type ?? 'fixed') === 'fixed')>Fixed amount</option><option value="percentage" @selected(old('discount_type', $draft?->discount_type) === 'percentage')>Percentage (%)</option></select></div>
    <div class="col-6"><label for="pos-discount-value" class="form-label">Discount value</label><input id="pos-discount-value" type="number" class="form-control" min="0" step="0.01" value="{{ old('discount_value', $draft?->discount_value ?? 0) }}"></div>
    <div class="col-12"><div class="form-text">Applied to the entire sale before payment.</div><div id="pos-discount-error" class="small text-danger" role="alert"></div></div>
</div>
<div class="d-flex justify-content-between mb-2"><span>Subtotal</span><strong id="pos-subtotal">$0.00</strong></div><div class="d-flex justify-content-between mb-2"><span>Discount</span><strong id="pos-discount-amount">−$0.00</strong></div><div class="d-flex justify-content-between fs-5"><strong>Total</strong><strong id="pos-total" style="color:var(--brand)">$0.00</strong></div><div class="d-grid gap-2 mt-3"><button class="btn btn-outline-primary" id="pos-save-draft" type="button"><i class="bi bi-pause-circle me-1"></i>{{ $draft ? 'Update draft' : 'Hold as draft' }}</button><button class="btn text-white" id="pos-checkout" type="button" style="background:var(--brand)"><i class="bi bi-credit-card me-1"></i>Complete sale</button></div></div>
    </div></div>
</div>
...
@if($draft || old('items'))
<div id="pos-draft-data" data-payment="{{ old('payment_method_id', $draft?->payment_method_id) ?: (old('payment_method', $draft?->payment_method) === 'Due' ? 'due' : '') }}" data-items='@json(old('items', $draftItems))'></div>
@endif
@endsection
