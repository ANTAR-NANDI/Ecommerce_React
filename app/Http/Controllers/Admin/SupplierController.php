...
class SupplierController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse { $request->merge(['is_active'=>$request->boolean('is_active')]); $supplier=Supplier::create($request->validate(['name'=>'required|max:150','phone'=>'required|max:30','email'=>'nullable|email','address'=>'nullable','photo_media_id'=>'nullable|exists:media,id','is_active'=>'boolean'])); app(AccountService::class)->ensureSupplierHead($supplier); return to_route('admin.suppliers.index')->with('success','Supplier created successfully with an accounts payable ledger head.'); }
