...
class PublicCheckoutController extends Controller
{
    public function coupon(Request $request): JsonResponse
    {
        ...
        if (! $promotion || ($promotion->minimum_order_amount && $data['subtotal'] < $promotion->minimum_order_amount) || ($promotion->usage_limit && $promotion->used_count >= $promotion->usage_limit)) {
            return response()->json(['message' => 'This coupon is not available for the current order.'], 422);
        }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $items = collect($data['items'])->map(function ($item) use ($products) {
            ...
            if ($product->discount > 0) {
                $price = $product->discount_type === 'percent' ? $price * (1 - $product->discount / 100) : max(0, $price - $product->discount);
            }

            return ['product_id' => $product->id, 'product_name' => $product->name, 'quantity' => $item['quantity'], 'unit_price' => round($price, 2), 'line_total' => round($price * $item['quantity'], 2)];
        });
        $subtotal = (float) $items->sum('line_total');
        $promotion = $data['coupon_code'] ? $this->couponFor($data['coupon_code']) : null;
        ...
        $order = DB::transaction(function () use ($data, $items, $subtotal, $discount, $promotion) {
            ...
            $order = EcommerceOrder::create(['order_number' => 'EC-'.now()->format('ymdHis').'-'.random_int(100, 999), 'customer_id' => $customer?->id, 'customer_name' => $data['name'], 'customer_phone' => $data['phone'], 'customer_email' => $data['email'], 'shipping_address' => trim(($data['address_tag'] ? ucfirst($data['address_tag']).': ' : '').$data['address'].($data['area'] ? ', '.$data['area'] : '')), 'warehouse_id' => null, 'payment_method' => $data['payment_method'] === 'card' ? 'Credit / Debit Card' : 'Cash on delivery', 'payment_status' => 'pending', 'status' => 'pending', 'shipping_charge' => 0, 'subtotal' => $subtotal, 'discount' => round($discount, 2), 'total' => round($subtotal - $discount, 2), 'customer_note' => $data['note']]);
        ...
        if ($order->customer_email) {
            try {
                ...
                Log::warning('Order confirmation email could not be sent.', ['order' => $order->order_number, 'exception' => $exception->getMessage()]);
            }
        }
    ...
    private function couponFor(string $code): ?Promotion
    {
        return Promotion::where('type', 'promo_code')->where('code', mb_strtoupper(trim($code)))->where('is_active', true)->where(fn ($q) => $q->whereNull('starts_at')->orWhere('starts_at', '<=', now()))->where(fn ($q) => $q->whereNull('ends_at')->orWhere('ends_at', '>=', now()))->first();
    }
}
