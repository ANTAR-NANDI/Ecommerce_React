...
class PublicCheckoutController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $order = DB::transaction(function () use ($data, $items, $subtotal, $discount, $promotion) { $order = EcommerceOrder::create(['order_number' => 'EC-'.now()->format('ymdHis').'-'.random_int(100, 999), 'customer_name' => $data['name'], 'customer_phone' => $data['phone'], 'customer_email' => $data['email'], 'shipping_address' => trim(($data['address_tag'] ? ucfirst($data['address_tag']).': ' : '').$data['address'].($data['area'] ? ', '.$data['area'] : '')), 'warehouse_id' => Warehouse::where('is_active', true)->value('id'), 'payment_method' => $data['payment_method'] === 'card' ? 'Credit / Debit Card' : 'Cash on delivery', 'subtotal' => $subtotal, 'discount' => round($discount, 2), 'total' => round($subtotal - $discount, 2), 'customer_note' => $data['note']]); $order->items()->createMany($items->all()); if ($promotion) $promotion->increment('used_count'); return $order; });
